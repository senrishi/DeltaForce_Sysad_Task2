...
COPY ./sysad-1-users.yaml /home/scripts
EXPOSE 5432
EXPOSE 80

#installing dependencies
RUN apt update && \
    apt install -y cron curl git nginx postgresql-16 postgresql-client-16 && \
    curl -L https://github.com/mikefarah/yq/releases/download/v4.44.1/yq_linux_amd64  > /usr/bin/yq && \
    chmod +x /usr/bin/yq

#running task1 and copying task4 
COPY ./task1.sh /home/scripts
RUN chmod +x /home/scripts/task1.sh \
    && /home/scripts/task1.sh 

#running task2
COPY ./task2.sh /temp_loc
COPY ./copy_task2.sh /home/scripts
RUN chmod +x /home/scripts/copy_task2.sh \
    && /home/scripts/copy_task2.sh
...
COPY ./nginx_conf.sh /home/scripts
